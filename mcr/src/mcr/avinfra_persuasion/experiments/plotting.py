...
def plot_typed_state_policy_gradient_field(
    metric_x: MetricName | str,
    metric_y: MetricName | str,
    game: object,
    state_name: str,
    ...
    *,
    ...
    """
    Plot a projected policy-gradient field for one state/type mask policy.

    Typed state-dependent mask policies have one multinomial distribution per
    state and receiver type. This projects one such distribution to the
    marginal disclosure coordinates ``P(reveal metric_x | state, type)`` and
    ``P(reveal metric_y | state, type)`` while holding the other distributions
    fixed at the game's current signaling scheme.
    """
    metric_x = MetricName.coerce(metric_x)
    metric_y = MetricName.coerce(metric_y)
    if metric_x == metric_y:
    ...
    state_order = tuple(getattr(game, "_state_order", ()))
    type_order = tuple(getattr(game, "_type_order", ()))
    all_masks = tuple(getattr(game, "_all_masks", ()))
    metric_order = tuple(getattr(game, "_metric_order", ()))
    if not state_order or not type_order or not all_masks or not metric_order:
        raise ValueError(
            "game must expose state, type, mask, and metric order metadata."
        )
    if state_name not in state_order:
        raise ValueError(f"Unknown state {state_name!r}.")
    if type_name not in type_order:
        raise ValueError(f"Unknown receiver type {type_name!r}.")
    if frozenset(metric_order) != frozenset({metric_x, metric_y}):
        raise ValueError(
            "Projected typed state policy gradient fields currently require "
            "exactly the two selected metrics."
        )
    if not hasattr(game, "_finite_difference_gradient"):
        raise ValueError("game must provide _finite_difference_gradient().")
    if not hasattr(game, "signaling_scheme"):
    ...
    state_index = state_order.index(state_name)
    type_index = type_order.index(type_name)
    masks_per_distribution = len(all_masks)
    row_index = state_index * len(type_order) + type_index
    row_start = row_index * masks_per_distribution
    row_stop = row_start + masks_per_distribution
    base_policy = game.signaling_scheme()
    ...
    for row_idx in range(grid_size):
        for col_idx in range(grid_size):
            ...
            probabilities = _typed_state_policy_with_independent_distribution(
                base_policy=base_policy,
                state_name=state_name,
                type_name=type_name,
                all_masks=all_masks,
            ...
            flat_logits = _typed_state_policy_logits(
                state_order=state_order,
            ...
            next_policy = game.signaling_scheme(logits=next_logits)
            next_point = _state_policy_marginal_point(
                next_policy[state_name][type_name],
                metric_x,
                metric_y,
            )
            force = next_point - current_point
    ...
    if result is not None:
        points = _typed_state_policy_points(
            metric_x,
            metric_y,
            state_name,
            type_name,
    ...
    _format_policy_axes(
        ...
        metric_x,
        metric_y,
        title=f"Policy gradient field: {state_name} / {type_name}",
    )
    return ax


def plot_typed_state_policy_gradient_fields(
    metric_x: MetricName | str,
    metric_y: MetricName | str,
    game: object,
    result: Mapping[str, object] | None = None,
    ...
    *,
    ...
    """Plot one projected gradient field for each state and receiver type."""
    state_order = tuple(getattr(game, "_state_order", ()))
    type_order = tuple(getattr(game, "_type_order", ()))
    if not state_order:
        raise ValueError("game must expose non-empty _state_order metadata.")
    if not type_order:
        raise ValueError("game must expose non-empty _type_order metadata.")

    field_count = len(state_order) * len(type_order)
    if axes is None:
        _, axes_array = plt.subplots(
            len(state_order),
            len(type_order),
            figsize=(6.2 * len(type_order), 5.7 * len(state_order)),
            squeeze=False,
        )
    else:
        axes_array = np.asarray(axes, dtype=object)
        if axes_array.size < field_count:
            raise ValueError("axes must contain at least one axis per state/type pair.")
    ...
    for ax, (state_name, type_name) in zip(flat_axes, state_type_pairs):
    ...
    for ax in flat_axes[field_count:]:
        ax.set_visible(False)
...
def _typed_state_policy_points(
    metric_x: MetricName,
    metric_y: MetricName,
    state_name: str,
    type_name: str,
    ...
    for policy in policy_history:
        if not isinstance(policy, Mapping) or state_name not in policy:
            raise ValueError(
                "Each typed state policy history entry must contain the "
                "requested state."
            )
        state_policy = policy[state_name]
        if not isinstance(state_policy, Mapping) or type_name not in state_policy:
            raise ValueError(
                "Each typed state policy history entry must contain the "
                "requested receiver type."
            )
        points.append(
            _state_policy_marginal_point(
                state_policy[type_name],
                metric_x,
                metric_y,
            )
        )
...
def _typed_state_policy_with_independent_distribution(
    *,
    base_policy: Mapping[str, Mapping[str, Mapping[frozenset[MetricName], float]]],
    state_name: str,
    type_name: str,
    all_masks: Sequence[frozenset[MetricName]],
    metric_x: MetricName,
    ...
    probabilities = {
        state: {
            receiver_type: dict(distribution)
            for receiver_type, distribution in type_distributions.items()
        }
        for state, type_distributions in base_policy.items()
    }
...
def _typed_state_policy_logits(
    *,
    state_order: Sequence[str],
    ...
    probabilities: Mapping[
        str,
        Mapping[str, Mapping[frozenset[MetricName], float]],
    ],
) -> np.ndarray:
    probability_values = np.asarray(
        [
            [
                [probabilities[state_name][type_name][mask] for mask in all_masks]
                for type_name in type_order
            ]
            for state_name in state_order
        ],
...
def plot_state_mask_policy(
    ...
    state name to either a mask-probability table or receiver-type
    mask-probability tables.
    """
    ...
    first_value = next(iter(first_distribution.values()))
    is_typed_policy = isinstance(first_value, Mapping)

    if is_typed_policy:
        row_keys = tuple(
            (state_name, str(type_name))
            for state_name in state_names
            for type_name in sorted(final_probabilities[state_name])
        )
        sample_state, sample_type = row_keys[0]
        sample_distribution = final_probabilities[sample_state][sample_type]
        if not isinstance(sample_distribution, Mapping) or not sample_distribution:
            raise ValueError(
                "Each typed state distribution in 'final_probabilities' must be "
                "a non-empty mapping."
            )
        masks = tuple(
            sorted(
                ...
                [
                    float(
                        _state_mask_probability(
                            final_probabilities[state_name][type_name],
                            mask,
                        )
                    )
                    for mask in masks
                ]
                for state_name, type_name in row_keys
            ],
            dtype=float,
        )
        yticklabels = [
            f"{state_name} / {type_name}" for state_name, type_name in row_keys
        ]
        ylabel = "State / type"
        title = "Typed state-dependent mask policy"
        colorbar_label = "P(mask | state, type)"
    else:
        masks = tuple(
            sorted(
                first_distribution,
        ...
        ylabel = "State"
        title = "State-dependent mask policy"
        colorbar_label = "P(mask | state)"

    row_count = len(yticklabels)
    width = max(6.0, 1.3 * len(masks))
    height = max(3.0, 0.8 * row_count + 1.5)
